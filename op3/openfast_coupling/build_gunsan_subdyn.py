"""
Build Gunsan 4.2MW SubDyn Input File from SSOT
===============================================
...
def write_subdyn(output_path, ssi_files=None):
    ...
    w("Gunsan 4.2MW UNISON U136 Tripod Suction Bucket - Generated from SSOT_REAL_FINAL.txt")
...
def build_gunsan_fst(output_dir, subdyn_name, tmax=120, comp_aero=0, comp_inflow=0):
    """
    Rewrite the Gunsan .fst to match OpenFAST v4.0.2 format exactly.
    ...
    """
    fst_path = Path(output_dir) / "Gunsan-4p2MW.fst"
    ...
    ed_name = ed_match.group(1) if ed_match else "Gunsan-4p2MW_ElastoDyn.dat"

    content = f"""------- OpenFAST EXAMPLE INPUT FILE -------------------------------------------
Gunsan 4.2MW UNISON U136 Tripod Suction Bucket OWT (v4.0.2 compatible)
---------------------- SIMULATION CONTROL --------------------------------------
...
if __name__ == '__main__':
    ...
    print("=" * 65)
    print("  Build Gunsan 4.2MW SubDyn from SSOT")
    print("=" * 65)

    GUNSAN_DIR = Path(r"f:\TREE_OF_THOUGHT\PHD\openfast\Gunsan_4p2MW")
    RESULTS_DIR = COUPLING_OUTPUT
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)

    # --- Test 1: Fixed base SubDyn (verify SubDyn works) ---
    print("\n[1] Fixed base SubDyn (verification)")
    fixed_dir = RESULTS_DIR / "gunsan_fixed_v4"
    if fixed_dir.exists():
        shutil.rmtree(fixed_dir)
    shutil.copytree(GUNSAN_DIR, fixed_dir)

    write_subdyn(
        fixed_dir / "Gunsan-4p2MW_SubDyn.dat",
        ssi_files=["", "", ""]
    )
    build_gunsan_fst(fixed_dir, "Gunsan-4p2MW_SubDyn.dat", tmax=30)

    # --- Test 2: Full 6x6 SSI (Path A) ---
    print("\n[2] Path A: Full 6x6 SSI")
    ssi_dir = RESULTS_DIR / "gunsan_pathA_v4"
    if ssi_dir.exists():
        shutil.rmtree(ssi_dir)
    shutil.copytree(GUNSAN_DIR, ssi_dir)
    ...
    write_subdyn(
        ssi_dir / "Gunsan-4p2MW_SubDyn.dat",
        ssi_files=["SSI_bucket1.dat", "SSI_bucket2.dat", "SSI_bucket3.dat"]
    )
    build_gunsan_fst(ssi_dir, "Gunsan-4p2MW_SubDyn.dat", tmax=30)

    # --- Test 3: Diagonal-only SSI (for comparison) ---
    print("\n[3] Diagonal SSI (comparison baseline)")
    diag_dir = RESULTS_DIR / "gunsan_diagonal_v4"
    if diag_dir.exists():
        shutil.rmtree(diag_dir)
    shutil.copytree(GUNSAN_DIR, diag_dir)
    ...
    write_subdyn(
        diag_dir / "Gunsan-4p2MW_SubDyn.dat",
        ssi_files=["SSI_bucket1.dat", "SSI_bucket2.dat", "SSI_bucket3.dat"]
    )
    build_gunsan_fst(diag_dir, "Gunsan-4p2MW_SubDyn.dat", tmax=30)
