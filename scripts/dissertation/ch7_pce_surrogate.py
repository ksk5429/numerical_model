"""
Chapter 7 / Task E: Hermite polynomial chaos surrogate of the Gunsan
...
coordinate xi to the Gunsan f1 response, with the physical scour
...
def gunsan_forward_direct(xi: float) -> float:
...
def main():
    ...
    print()
    print("=" * 72)
    print(" Op3 Chapter 7 Task E -- PCE surrogate of Gunsan forward")
    print("=" * 72)
    ...
    t0 = time.time()
    pce = build_pce_1d(gunsan_forward_direct, order=6)
    ...
    direct = np.array([gunsan_forward_direct(float(x)) for x in xi_test])
    ...
    t0 = time.time()
    _ = [gunsan_forward_direct(float(x)) for x in xi_bench[:n_bench]]
    ...
    # --- 4. Output ---
    result = {
        "task": "E - PCE surrogate of Gunsan forward model",
        "parameterisation": {
            "s_mean_m": S_MEAN_M,
            "s_sigma_m": S_SIGMA_M,
    ...
    out = REPO_ROOT / "PHD/ch7/gunsan_pce_surrogate.json"
