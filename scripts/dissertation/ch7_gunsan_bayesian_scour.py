"""
Chapter 7 / Task A: Bayesian identification of Gunsan scour depth
...
The dissertation defense question is:
    "Given a field-measured first natural frequency f1_meas, what is
     the posterior distribution of the scour depth s at the Gunsan
     4.2 MW tripod suction-bucket foundation?"

This script answers it by:

  1. Building the Op^3 Gunsan example forward model that maps scour
  ...
  3. Running ``op3.uq.bayesian.grid_bayesian_calibration`` with the
     Gunsan field-measured f1 = 0.244 Hz and a measurement sigma
...
Gunsan 4.2 MW tripod OWT, 20,039 RANSAC windows, f1 = 0.244 Hz.

Run
---
    python scripts/dissertation/ch7_gunsan_bayesian_scour.py
    python scripts/dissertation/ch7_gunsan_bayesian_scour.py --sigma 0.005
"""
...
GUNSAN_F1_MEASURED_HZ = 0.244
GUNSAN_F1_SIGMA_HZ = 0.003   # typical RANSAC OMA scatter per PhD Ch.5


def gunsan_forward(scour_depth_m: float) -> float:
    """
    Op^3 Gunsan forward model: scour depth -> first natural frequency.
    Rebuilds the Gunsan example each time with the given scour depth
...
def main():
    ...
    ap.add_argument("--measured", type=float, default=GUNSAN_F1_MEASURED_HZ,
                    help="Field-measured f1 in Hz (default 0.244 per Ch. 5)")
    ap.add_argument("--sigma", type=float, default=GUNSAN_F1_SIGMA_HZ,
    ...
    ap.add_argument("--out", default="PHD/ch7/gunsan_bayesian_scour.json",
    ...
    print()
    print("=" * 72)
    print(" Op3 Chapter 7 Task A -- Gunsan Bayesian scour identification")
    print("=" * 72)
    ...
    for i, s in enumerate(grid):
        preds[i] = gunsan_forward(float(s))
        if i % 10 == 0:
    ...
    result = {
        "task": "A - Gunsan Bayesian scour identification",
        "measurement": {
            "f1_Hz": args.measured,
            "sigma_Hz": args.sigma,
        ...
        "forward_model": {
            ...
            "example": "04_gunsan_4p2mw_tripod",
    ...
    print()
    print("=" * 72)
    print(f" Posterior on Gunsan scour depth s")
    print("=" * 72)
