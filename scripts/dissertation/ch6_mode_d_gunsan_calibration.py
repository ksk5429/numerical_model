...
calibration on the Gunsan 4.2 MW tripod.
...
Strategy
--------
1. Load the Gunsan dissipation profile (data/fem_results/dissipation_profile.csv).
...
Run
---
    python scripts/dissertation/ch6_mode_d_gunsan_calibration.py
"""
...
GUNSAN_F1_SIGMA_HZ = 0.003


def gunsan_mode_d_forward(alpha: float, beta: float) -> float:
    """Op^3 Gunsan Mode D forward model."""
...
def main():
    ...
    ap.add_argument("--out", default="PHD/ch6/gunsan_mode_d_calibration.json")
    args = ap.parse_args()
    ...
    print()
    print("=" * 72)
    print(" Op3 Chapter 6 Task B -- Mode D Gunsan calibration")
    print("=" * 72)
    ...
    for i, a in enumerate(alpha_grid):
        for j, b in enumerate(beta_grid):
            try:
                preds[i, j] = gunsan_mode_d_forward(float(a), float(b))
    ...
    result = {
        "task": "B - Mode D Gunsan calibration",
        "measurement": {
            "f1_Hz": args.measured,
            "sigma_Hz": args.sigma,
    ...
    print()
    print("=" * 72)
    print(f" Mode D MAP estimate for Gunsan")
    print("=" * 72)
