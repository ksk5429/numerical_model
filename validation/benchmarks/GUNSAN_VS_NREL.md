# Gunsan 4.2 MW vs NREL Reference Library — Side-by-Side Comparison

This document places the Gunsan 4.2 MW tripod suction-bucket turbine
...
glance which NREL model is the closest analog to Gunsan for any
...
- **Read from the Gunsan SSOT** (`op3/config/gunsan_site.yaml`,
  marked 🔧).
...
| Property                      | NREL 5MW (r-test) | NREL OC3 Monopile | NREL 2.8-127 | NREL 1.72-103 | **Gunsan 4.2 MW** |
|-------------------------------|:-----------------:|:------------------:|:------------:|:-------------:|:------------------:|
...
1. **Gunsan is the only turbine in this comparison with a
   ...
   foundation-response comparisons, the Gunsan model has no direct
   NREL equivalent — the NREL reference library does not include a
   tripod suction bucket.

2. **Gunsan has the largest rotor diameter** in the comparison at
   ...
   Gunsan rates at 4.2 MW not 5 MW because the Unison U136
   generator is rated below the NREL 5MW design.

3. **Gunsan operates at the highest rotor speed** (13.2 rpm) in the
...
4. **Gunsan hub height (96.3 m) is the tallest.** The tower is
...
5. **Foundation stiffness dominates the frequency difference.**
   ...
   seabed), it sits between the fixed-base NREL 5MW and the Gunsan
   tripod in first-mode frequency. Gunsan's tripod is substantially
...
+ foundation model (for Gunsan in Mode C — distributed BNWF).
...
| **Gunsan 4.2 MW (Op³, C)** | **0.244 Op³** | **0.236 Op³** | **0.76 Op³**       | **1.28 Op³**             |

**How to read this:**

- Gunsan's first fore-aft frequency at 0.244 Hz is **25% lower**
  ...
  magnitude as the difference between the OC3 monopile and Gunsan
...
- Gunsan's first tower bending (second global mode) at 1.28 Hz is
...
**Note on the Gunsan blade inheritance.** The current Gunsan OpenFAST
...
updated to match the real Unison U136 specification. Specifically:

- The `TipRad` parameter in `Gunsan-4p2MW_ElastoDyn.dat` is still
...
`gunsan_4p2mw/openfast_deck/README.md`. The aerodynamic predictions
from the current deck are therefore not representative of the real
Unison U136 rotor. The **structural predictions** (eigenmodes, tower
...
| Property             | NREL 5MW                | **Gunsan 4.2 MW**        |
|----------------------|:-----------------------:|:-------------------------:|
...
**Observation:** Despite Gunsan's smaller base diameter (4.2 m vs
...
combined with the taller hub makes the Gunsan tower more slender
than the NREL 5MW, contributing to the lower first-mode frequency.

## Foundation comparison

| Property              | NREL OC3 Monopile          | **Gunsan 4.2 MW Tripod**         |
|-----------------------|:-------------------------:|:--------------------------------:|
| Configuration         | Single pile               | **Three caissons at 120° spacing** |
| Outer diameter        | 6.0 m 📄                  | **8.0 m per caisson 🔧**         |
| Embedment depth       | 36 m below seabed 📄      | **9.3 m skirt length per caisson 🔧** |
...
| Soil at Gunsan        | N/A                        | **Undrained clay, su(z) = 15 + 20z kPa 🔧** |
...
Gunsan tripod uses three short-stubby caissons that develop lateral
...
The Gunsan model has been compared to the NREL reference library
...
|---------------------------------|-------------------|
| **Rotor aerodynamics**          | Gunsan inherits NREL 1.72-103 rotor as template; agreement is by construction |
| **Tower structural dynamics**   | Gunsan tower modeled independently with Op³ tower properties; first FA frequency 12-25% lower than NREL 5MW due to taller/softer tower and softer foundation |
| **Foundation SSI**              | **No NREL equivalent** — Gunsan tripod is the unique feature; independently validated against centrifuge experiments (1.19% kill-test error) |
| **Aero-elastic coupling**       | Not yet validated on Gunsan (ROSCO not tuned); NREL 5MW OC3 monopile used as benchmark for the SubDyn coupling bridge |
| **Simulated vs measured frequencies** | Gunsan OpenFAST prediction at 0.244 Hz matches field-measured 0.244 Hz from 32 months of nacelle accelerometer data — **agreement is within field-data scatter** |
